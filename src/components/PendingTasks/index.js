...
const PendingTasks = (props) => {
    ...
    const handledate = (event) => {
        const formatedDate = format(new Date(event.target.value),'dd/MM/yyyy' )
        console.log(formatedDate, "chjnbg vbhjtjgujuy hg jhb,bki")
        setEditedTaskDate(formatedDate)
    }
    ...
    return (
        <li className={`todoitem-container ${themebackgroundcolor} `}>
                ...
                <div className="large-screen-task-item">
                    <div className="todo-title-container">
                        {editId === id? 
                            <>
                                <div>
                                    <input autoFocus className={theme ? "dark-search-bar search-bar" : "search-bar"} type="text" maxLength={30} value = {editedTaskTitle} placeholder="Edit Task" onChange={(event) => setEditedTaskTile(event.target.value)} onKeyUp={titleok}/>
                                    ...
                                    <input ref={inputdate} className={theme ? "dark-date-input-field search-bar" : "search-bar date-input-field"} type="date" value = {editedTaskDate} placeholder="date" onChange={handleDelete}/>
                                </div>
                            </> 
                            : 
                            <div className="task-details-conatiner">
                                <h5 className={theme ? "dark-todo-title" : "todo-title" }>{title}</h5>
                                <p className={theme ? "dark-todo-description" : "todo-description" }>{note}</p>
                            </div>
                        }
                    </div>
                    <div className="large-screen-edit-date-delete-check-options">
                        <div className="title-date">
                            ...
                            <button onClick={handleTodoStatus} className="icons">
                                <IoIosCheckmarkCircleOutline className = {theme ? "dark-checkbox-icon" : "icon"} />
                            </button>
                        </div>
            ...
            <div className="small-screen-options">
                <div className="todo-title-container">
                    ...
                    <div>
                        ...
                        <TextareaAutosize ref={smallInputNote} className={theme ? "dark-search-bar search-bar" : "search-bar"}  type="text" value = {editedTaskNote} placeholder="edit task description...." onChange={(event) => setEditedtaskNote(event.target.value)} onKeyUp={smallnoteok}/>
                        <input ref={smallInputdate} className={theme ? "dark-date-input-field search-bar" : "search-bar date-input-field"} type="date" value = {editedTaskDate} placeholder="date" onChange={(event) => setEditedTaskDate(event.target.value)}/>
                    </div>
                    </> 
                    : 
                    <div className="task-details-conatiner ">
                ...
                {editId !== id && <p className={theme ? "dark-todo-description" : "todo-description" }>{note}</p> }
